...
// Mirrors the approach from the Pathfinder Prototype V2 macro but integrated
// into the persistent system.
// ============================================================================
(() => {
  ...
  function buildPseudoScriptSource() {
    // Injected into game.ONI.pseudo.play as a serialised script string
    return `
...
try{
  ...
  DP.Movement = {
    /**
     * Animate the token to destNode using pseudo-animation (visual-only clone),
     * then update the real token document.
     *
     * @param {Token} token   - Foundry Token placeable
     * @param {object} destNode - graph node (has .center.x/y)
    ...
    async moveToNode(token, destNode) {
      ...
      const realX  = Math.round(toC.x - tw / 2);
      const realY  = Math.round(toC.y - th / 2);
      ...
      if (typeof pseudoApi === "function") {
        ...
        // Fallback: simple teleport if pseudo-animation API is absent
    ...
    /**
     * Revert the token back to its previous position (after player declines).
     * @param {Token} token
     * @param {{ x: number, y: number }} savedPos  - top-left document coords before move
     */
    ...
    /**
     * Save current token document position (top-left x/y) before moving.
     */
    savePosition(token) {
