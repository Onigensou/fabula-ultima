...
// (.total/.pass/.isFumble) or null if the Check Requester isn't available.
async function rollInitiativeCheck(actor, dl, extraMods = []) {
  ...
  const results = await CR.request([actor], {
    ...
    allowInvokes: false,
    modifiers,
    context: { checkContext: "init", initiativeGroupCheck: true },
    checkBuffActions: ["initiative"],
  });
...
// Dev/verify helper — run ONE initiative check for a single actor and return the
// full CheckResult (incl. modifierParts) so the tagged-modifier wiring
// (check_mod_all/init/dex/ins, gear check_buff, initiative penalty) can be
// inspected end-to-end. Not used in production.
export async function debugInitiativeCheck(actorUuid, dl = 10) {
  const doc = await fromUuid(actorUuid).catch(() => null);
  const actor = doc?.documentName === "Actor" ? doc : (doc?.actor ?? null);
  if (!actor) return { error: "actor not resolved", actorUuid };
  const result = await rollInitiativeCheck(actor, dl);
  return { actor: actor.name, dl, result };
}
