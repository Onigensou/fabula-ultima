...
//
// Flow (pre/post):
//   pre  — GM text dialog + whisper toggle; returns { text, whisper }
//   post — resolves whisper targets and posts the styled chat card
// ============================================================================
(() => {
  ...
  Hooks.once("ready", () => {
    window["oni.OppEffectRegistry"]?.register("information", {

      // ── Pre phase: GM enters the discovered detail ─────────────────────────
      async pre(ctx) {
        console.debug(TAG, "[entry]", { actorUuid: ctx.actorUuid, actorName: ctx.actorName });

        console.debug(TAG, "[pre] opening GM text dialog...");
        ...
        console.debug(TAG, "[pre] dialog result:", detail ? { textLength: detail.text.length, whisper: detail.whisper } : "NULL");
        if (!detail) { console.debug(TAG, "[pre] dialog cancelled or empty"); return null; }

        return { text: detail.text, whisper: detail.whisper };
      },

      // ── Post phase: post the styled chat card ──────────────────────────────
      async post(ctx, preResult) {
        const { text, whisper } = preResult ?? {};
        ...
        if (whisper) {
          ...
          console.debug(TAG, "[post] whisper targets:", whisperIds, "| ownerId:", ownerId);
        } else {
          console.debug(TAG, "[post] posting publicly");
        }
