...
(() => {
  ...
  Hooks.once("ready", () => {
    window["oni.OppEffectRegistry"]?.register("progress", async (ctx) => {
      ...
      console.debug(TAG, "[step 1] window.clockDatabase:", db ? "FOUND" : "NOT FOUND");
      if (!db) {
        ui.notifications?.warn("[Opportunity] Global Progress Clocks module not active.");
        console.warn(TAG, "[exit] window.clockDatabase missing");
        return;
      }

      const clocks = [];
      db.forEach(c => clocks.push(c));
      console.debug(TAG, "[step 1] clocks found:", clocks.map(c => `${c.name} (${c.value}/${c.max})`));
      if (!clocks.length) {
        ui.notifications?.warn("[Opportunity] No clocks found.");
        console.warn(TAG, "[exit] no clocks");
        return;
      }

      console.debug(TAG, "[step 2] opening clock picker dialog...");
      ...
      console.debug(TAG, "[step 2] dialog result:", result ? { clockName: result.clock?.name, delta: result.delta } : "NULL");
      if (!result) { console.debug(TAG, "[exit] dialog cancelled"); return; }

      const { clock, delta } = result;
      if (delta === 0) { console.debug(TAG, "[exit] delta is 0, no change"); return; }

      const newValue = Math.max(0, Math.min(clock.max, clock.value + delta));
      console.debug(TAG, "[step 3] updating clock:", clock.name, "|", clock.value, "→", newValue);
      if (newValue === clock.value) {
        ui.notifications?.info(`[Opportunity] "${clock.name}" is already at its limit.`);
        console.debug(TAG, "[exit] clock already at limit");
        return;
      }

      await db.update({ id: clock.id, value: newValue })
        .catch(e => console.error(TAG, "[step 3] clockDatabase.update failed:", e));

      console.debug(TAG, "[step 4] posting result card...");
      await postProgressCard({ actorName: ctx.actorName, clockName: clock.name, oldValue: clock.value, newValue, max: clock.max });
