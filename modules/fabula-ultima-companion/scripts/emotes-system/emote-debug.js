/**
 * emote-debug.js
 * Fabula Ultima Companion - Emote System Debug Utility
 * Foundry VTT v12
 *
 * Purpose:
 * - Centralize all debug logging for the Emote System
 * - Give all future Emote scripts one shared logger
 * - Make it easy to toggle debug on/off from Foundry settings or console
 *
 * Console examples:
 *   const DBG = globalThis.__ONI_EMOTE_DEBUG__;
 *   DBG.enable();
 *   DBG.log("Bootstrap", "Emote System initialized");
 *   DBG.group("Resolver", "Resolved emote target", { tokenId: "abc123" });
 *   DBG.verbose("Hotkeys", "Alt+1 detected", { slot: "1" });
 *   DBG.disable();
 */

(() => {
  const GLOBAL_KEY = "__ONI_EMOTE_DEBUG__";
  ...
  const SETTINGS = {
    ENABLED: "emoteDebugEnabled",
    VERBOSE: "emoteDebugVerbose"
  };
  ...
  const api = {
    ...
    async enable() {
      ...
      console.info("[Emote:Debug] Debug logging enabled");
      return true;
    },
    ...
    async disable() {
      ...
      console.info("[Emote:Debug] Debug logging disabled");
      return true;
    },
    ...
    async setVerbose(enabled) {
      ...
      console.info(`[Emote:Debug] Verbose logging ${enabled ? "enabled" : "disabled"}`);
      return true;
    },
  ...
  Hooks.once("init", () => {
    game.settings.register(MODULE_ID, SETTINGS.ENABLED, {
      name: "Emote System Debug Logging",
      hint: "Enable debug logs for the Emote System.",
      scope: "client",
      config: true,
      type: Boolean,
      default: false
    ...
    game.settings.register(MODULE_ID, SETTINGS.VERBOSE, {
      name: "Emote System Verbose Logging",
      hint: "Show extra detailed logs for the Emote System.",
      scope: "client",
      config: true,
      type: Boolean,
      default: false
  ...
  Hooks.once("ready", () => {
    try {
      ...
      globalThis.FUCompanion.api.EmoteDebug = api;
    } catch (err) {
      console.warn("[Emote:Debug] Failed to attach API to FUCompanion.api", err);
    }

    api.verbose("Bootstrap", "emote-debug.js ready", {
      moduleId: MODULE_ID,
      systemId: SYSTEM_ID,
