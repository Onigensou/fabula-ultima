...
(() => {
  ...
  // Wind gust schedule — warnAt / hitAt are ms from game start
  const GUST_SCHEDULE = [
    { warnAt:  5_500, hitAt:  6_200 },
    { warnAt: 10_000, hitAt: 10_700 },
    { warnAt: 13_300, hitAt: 14_000 },
  ];
  ...
  function _scheduleGustVisuals() {
    for (const gust of GUST_SCHEDULE) {
  ...
  function _startMinigame(actorId, actor) {
    ...
    // Init gust tracking (mutable per-game state)
    _gusts = GUST_SCHEDULE.map(g => ({ ...g, hit: false }));
