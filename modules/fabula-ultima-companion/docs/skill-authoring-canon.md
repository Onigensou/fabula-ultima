...
| "Fires ONLY when THIS skill/weapon is the acting one" (a self-rider — e.g. a monster attack's on-hit effect, Fiery Onslaught's blaze_chain) | reaction row sets `reaction_source_skill: "<this item's exact name>"` — matched against `payload.sourceSkillName` (the acting skill/weapon name). Without it the row is **ambient** and fires on EVERY qualifying action by the owner. | ~~rely on `skill_type` to scope it~~ — the skill_type reaction-gate was REMOVED 2026-06-15; `skill_type` no longer affects whether a reaction fires. An "Active" skill with an unscoped on-hit row now leaks onto basic attacks. |
