...
| `ATTACK_VS_DEF` / `ATTACK_VS_MDEF` | **DEFENSE-AXIS gate.** Which Defense the in-flight attack's accuracy Check resolves against — the "strike" (vs DEF) vs "magic" (vs MDEF) axis the damage engine + card labels use (the SAME `resolvesVsMagicDefense` resolution: explicit per-item `defense_target_type` wins, else Spell → MDEF). Read from `payload.defenseResolved` (`"def"` \| `"mdef"` \| `null`), threaded onto the `creature_targeted_by_action` payload. **Both return 0 when no accuracy Check was rolled** (auto-hit / non-Check action → `defenseResolved` is null), so a `== 1` gate fails closed and a defensive reaction stays dormant on beneficial no-Check targeting. Used by Matador's **Verónica** (`ATTACK_VS_DEF == 1`) so its +2 DEF only fires against attacks it can actually mitigate — skipping offensive spells and `mdef`-tagged weapon Attacks (Arc Wand) that resolve vs Magic Defense. |
