...
| `TRIGGER_AMOUNT` | The SIZE of the resource change that fired a **resource-ledger** trigger — `payload.amount`. On `creature_lose_resource` it is how much HP/MP/IP the subject just lost (damage, DoT tick, cost or drain alike — narrow with `reaction_cause_filter`); on `creature_gain_resource`, how much was restored. Always >= 0; the direction lives in the trigger name. 0 on every other trigger. Use it for "react in PROPORTION to what you lost" (Drakoza's Drako Fury banks damage taken as Fury charges). |
