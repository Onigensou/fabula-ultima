...
// Macro names must match your macro list exactly
  ...
  // devMode is read from payload.options.devMode after the prompt runs.
  // The Battle Prompt dialog has the checkbox and persists the choice via game.user flag.
  let devMode = false;
...
// Read dev mode from the payload written by the Battle Prompt checkbox
devMode = Boolean(payload?.options?.devMode);
log("devMode:", devMode);
...
pWithLean.context = { ...(pWithLean.context ?? {}), lean: devMode };
await writePayload(sourceScene, pWithLean);
payload = pWithLean;
log("lean set explicitly to:", devMode);
...
if (String(payload?.options?.battleSystem ?? "legacy") === "director") {
  ...
  // Clean lean from the flag now that the Director Manager has already read it
