...
// - Dev Mode checkbox added to Options section; persists via game.user flag
//
// Storage (Scene Flag):
//   scope: "world"
...
(async () => {
  ...
  // -----------------------------
  const encounterTablePick = pickFlag(`${DUNGEON_ROOT}.encounterTable`);
  const enemiesTablePick   = pickFlag(`${DUNGEON_ROOT}.enemiesTable`);
  ...
  // UI Definitions
  // -----------------------------
  const DEFAULT_FORMATION_PRESET = "party_line__enemy_rows";
  ...
  // Load persisted dev mode preference (per user, persists across sessions)
  const devModeDefault = Boolean(game.user?.getFlag?.("fabula-ultima-companion", "bdDevMode") ?? false);
...
<div data-bi-manualblock style="display:none; padding:10px 12px; border:1px solid var(--color-border-light-primary); border-radius:10px;">
  ...
  <div style="font-size:12px; opacity:0.8; margin-top:10px;">
    ...
    <details>
      ...
      <!-- Dev Mode — persists across sessions via game.user flag -->
      <div style="margin-top:8px; padding:8px 10px; background:rgba(122,155,182,0.08); border-radius:8px; border:1px solid var(--color-border-light-primary);">
        <label style="display:flex; align-items:center; gap:8px; font-weight:900; cursor:pointer;">
          <input type="checkbox" name="devMode" ${devModeDefault ? "checked" : ""} />
          ⚡ Dev Mode (skip preload &amp; cinematics)
        </label>
        <div style="font-size:12px; opacity:0.8; margin:4px 0 0 26px; line-height:1.35;">
          Skips screen crack, curtain, BGM, entrance dash, and BATTLE START banner for faster iteration.
  ...
  async function buildAndStorePayload(html) {
    ...
    // Dev Mode — read from checkbox and persist for next session
    const devMode = Boolean(fd.get("devMode"));
    if (game.user?.setFlag) {
      await game.user.setFlag("fabula-ultima-companion", "bdDevMode", devMode);
    }
    ...
    const payload = {
      ...
      party: {
        ...
        source: {
          ...
          resolvedAt: PARTY_DB.resolvedAt ?? nowIsoTs
        },
