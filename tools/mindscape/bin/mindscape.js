...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--party-archetype") out.partyArchetype = next();
...
Mindscape — offline Monte Carlo balance runs (game must be CLOSED)
  ...
  --party-archetype  standard | double-caster | no-healer | physical — replace the
                 Current Game party with a blank-slate one built by the rulebook's
                 character-creation rules (ruleset Part 6g).
...
`);
  ...
  // --party-archetype replaces the Current Game party with a blank-slate one built by the
  // character-creation rules (lib/archetype-party.js). World items and folders are loaded
  // here once and reused by --equip / --baseline-gear below.
  let worldItemsCache = null, foldersCache = null, game, party;
  if (args.partyArchetype) {
    worldItemsCache = await loadWorldItems();
    foldersCache = await loadWorldFolders();
    const level = Number.isFinite(args.level) ? args.level : 41;
    const power = args.power ?? "table";
    const k = Number.isFinite(args.skillLayerK) ? args.skillLayerK : DEFAULT_SKILL_LAYER_K;
    party = buildArchetypeParty(args.partyArchetype, {
      level, power, k, catalogue: basicCatalogue(worldItemsCache, foldersCache),
    });
    game = { gameName: "Blank-slate archetypes", partyName: `${args.partyArchetype} · L${level} · ${power} power` };
  } else {
    game = await resolveCurrentGame();
    party = await loadParty({ partyName: args.partyName });
  }
  const enemies = [
  ...
  if (args.partyArchetype) {
    console.log(`⚠ ARCHETYPE PARTY — generated by the character-creation rules, not loaded from the world. Generic kits; no class-skill identities.`);
    for (const p of party) {
      const a = p.fromArchetype;
      console.log(`    ${p.name.padEnd(10)} ${a.book}  ·  DEX d${a.dice.dex} INS d${a.dice.ins} MIG d${a.dice.mig} WLP d${a.dice.wlp}`
        + `  ·  HP ${p.hp.max} MP ${p.mp.max} DEF ${p.def} MDEF ${p.mdef}${a.layer ? `  ·  skill layer +${a.layer.accuracy} acc / +${a.layer.damage} dmg` : ""}`);
    }
  }
  ...
  if (args.equips?.length || args.unequips?.length || args.baselineGear?.length) {
    const worldItems = worldItemsCache ?? await loadWorldItems();
    ...
    if (args.baselineGear?.length) {
      const folders = foldersCache ?? await loadWorldFolders();
