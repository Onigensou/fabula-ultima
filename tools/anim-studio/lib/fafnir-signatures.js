...
function cruelUltimatum(opts = {}) {
  const cfg = Object.assign({
    ...
    /* P02 she leaves the board */
    riseMs: 900, riseFrac: 1.25,        // riseFrac: viewport heights climbed
    /* P03 fade to black */
    blackMs: 700, blackHoldMs: 200,
    /* P04 the sky arrives */
    skyFromFrac: 0.45, skySlideMs: 1100,
    /* P05 hold, with lightning */
    skyHoldMs: 1600,
    boltCount: 4, boltGapMin: 320, boltGapMax: 560, boltScale: 0.85, boltY: 0.38,
    /* P06 dim the curtain */
    ...
    /* P10 the fake dolly */
    dollyMs: 1100, dollyCutinScale: 0.62, dollyBeamScale: 1.9,
    dollyDriftX: 0.06, dollyDriftY: 0.10,
    /* P11-P12 whiteout, and the snap it hides */
    whiteInMs: 800, whiteHoldMs: 350, camSnapMs: 16,
    /* P13-P14 back to the board */
    sceneInMs: 900, beatMs: 300,
    /* P15 dim */
    targetDimTo: 0.55, targetDimMs: 400,
    /* P16 impact */
    strikeWidth: 0.10, strikeInMs: 260, strikeOutMs: 700,
  ...
  const body = [
    ...
    "",
    "// ── Layering ──",
    "// The Zero Power sky is a CURTAIN, not a dim. It has to hide the board,",
    "// which puts it ABOVE the tokens -- and that is exactly why P06's dim",
    "// cannot use oni.sceneDim, which seats itself below tokens by design and",
    "// would do nothing to a plate drawn over them. The takeover lives in one",
    "// host ordered by zIndex; the whiteout gets a layer of its OWN so that",
    "// tearing the curtain down does not tear the whiteout down with it.",
    "//",
    "//   host   5 caster clone   10 black   20 sky   30 bolts",
    "//         35 curtain dim    40 cut-in  45 charge  50 beam",
    "//   above 90 whiteout",
    ...
    "let torn = false;",
    "",
    "// Curtain art is authored in SCREEN space but lives in a WORLD-space layer,",
    "// so each plate re-derives its transform every tick. Pinning once would be",
    "// enough while the camera is parked and would break the instant anything",
    "// nudged it. Two lists: host pins die at teardown, top pins outlive it,",
    "// because the whiteout is still on screen while the curtain is destroyed.",
    "const pins = [];",
    "const pinsTop = [];",
    ...
    "  // Overdrawn 1.2x so a resize mid-shot cannot expose an uncovered edge.",
    ...
    "",
    "/* ── P02  She leaves the board ────────────────────────────────────────── */",
    "// hideToken re-asserts every tick: a cull pass will otherwise flip the real",
    "// token back on mid-shot, behind the curtain, and it would be waiting there",
    "// when the whiteout lifts.",
    "const clone = oni.cloneToken(caster, { parent: host });",
    ...
    "",
    "/* ── P04  The Zero Power sky slides down and fades in ─────────────────── */",
    "let skyDrop = 0;",
    "const sky = await imageSprite(A.skyImg, 20);",
    "if (sky) {",
    "  const ta = (sky.texture.orig.width || 1) / (sky.texture.orig.height || 1);",
    "  pins.push(function () {",
    "    // COVER, not contain: the plate must never show a letterbox edge. The",
    "    // art is 16:9 against a wider viewport, so this crops top and bottom.",
    "    const va = S.W / S.H;",
    "    let w, h;",
    ...
    "await oni.tween({ from: 0, to: 1, duration: cfg.skySlideMs, ease: E.outQuad, onUpdate: (v) => {",
    "  skyDrop = v; if (sky) sky.alpha = v;",
    "} });",
    "",
    "/* ── P05  Hold. Lightning and thunder over the settled sky ────────────── */",
    "// Same clip and thunder the entrance uses, so her arrival and her Zero",
    "// Power share one storm. Fired and not awaited -- the strikes are allowed",
    "// to run on under the dim and the cut-in.",
    "(async function () {",
    ...
    "let cutinRise = 0;   // 0 = below the frame, 1 = seated",
    "let cutinScale = 1;",
    "let cutinDrift = 0;  // P10 only",
    ...
    "} });",
    "",
    "/* ── P08  Charging the breath ─────────────────────────────────────────── */",
    "// The mouth is a fraction of the CUT-IN, not of the screen, so the glow and",
    "// the beam origin follow the art through the slide and the dolly.",
    "function mouthPoint() {",
    ...
    "} });",
    "",
    "/* ── P09  The breath is unleashed ─────────────────────────────────────── */",
    "let beamGain = 0;    // emergence, 0..1",
    "let beamScale = 1;   // P10 growth",
    "const beam = new PIXI.Graphics();",
    "beam.zIndex = 50; beam.blendMode = PIXI.BLEND_MODES.ADD;",
    "host.addChild(beam);",
    "pins.push(function () {",
    "  beam.clear();",
    ...
    "  // A wedge, not a line: it has to widen with distance for the dolly in P10",
    "  // to read as travelling along the beam rather than just scaling up.",
    "  const wedge = function (color, a0, a1, alpha) {",
    "    beam.beginFill(color, alpha);",
    "    beam.moveTo(m.x + nx * a0, m.y + ny * a0);",
    "    beam.lineTo(m.x + ux * len + nx * a1, m.y + uy * len + ny * a1);",
    "    beam.lineTo(m.x + ux * len - nx * a1, m.y + uy * len - ny * a1);",
    "    beam.lineTo(m.x - nx * a0, m.y - ny * a0);",
    ...
    "  beamGain = v; charge.alpha = 1 - v * 0.6;",
    "} });",
    "",
    "/* ── P10  Ride the beam out ───────────────────────────────────────────── */",
    "// A FAKE camera move. The real camera is irrelevant here -- it is behind a",
    "// full-screen curtain -- so the travel is sold by scaling the beam up while",
    ...
    "// The camera SNAPS home here instead of gliding, which anywhere else in this",
    "// file would be the bug we keep fixing. It is invisible: the screen is solid",
    "// white for the whole move, so an eased return would be work nobody sees,",
    "// and it would delay the reveal by its own duration.",
    "torn = true;",
    "pins.length = 0;",
    "restoreCaster();",
    ...
    "  const c = ctr(v);",
    "  // Arrives on the SAME diagonal it left on, from off-frame.",
    "  const back = S.wLen(Math.hypot(S.W, S.H));",
    "  const from = { x: c.x - Math.cos(cfg.beamAngle) * back, y: c.y - Math.sin(cfg.beamAngle) * back };",
    "  lanes.push(streak(from.x, from.y, c.x, c.y, cfg.color, S.wLen(S.H * cfg.strikeWidth), strike));",
    "  lanes.push(streak(from.x, from.y, c.x, c.y, cfg.coreColor, S.wLen(S.H * cfg.strikeWidth * 0.4), strike));",
    "}",
    "for (const l of lanes) l.alpha = 0;",
    "playSfx('sfxImpact', 'sfxImpactVol');",
    "await oni.tween({ from: 0, to: 1, duration: cfg.strikeInMs, ease: E.outQuad,",
    "  onUpdate: (v) => { for (const l of lanes) l.alpha = v; } });",
    "",
    "for (const v of victims) {",
    "  const c = ctr(v);",
    "  if (A.boomWebm) {",
    ...
    "}",
    "",
    "// The damage lands HERE, on the impact -- same gate as every other shot.",
    "done();",
    "",
    "const flashing = oni.domFlash({ color: cfg.flashColor, alpha: cfg.flashAlpha,",
