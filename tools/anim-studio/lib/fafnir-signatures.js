...
function cruelUltimatum(opts = {}) {
  const cfg = Object.assign({
    /* P00 Zero Power cut-in. Fractions of the viewport, so the framing
       holds on any window; Rauben's original used raw pixels. */
    zpDimTo: 0.55, zpDimMs: 180, zpDimOutMs: 260,
    zpPortraitH: 0.90,                  // max height, viewport heights
    zpPortraitW: 0.55,                  // max width, viewport widths
    zpInsetX: 0.09,                     // seated left edge, viewport widths
    zpBottomY: 0.84,                    // where her feet sit
    zpInMs: 340, zpHoldMs: 900, zpOutMs: 340,
    ...
    /* P05 hold. No strikes: they fought the charge cue that follows. */
    skyHoldMs: 2800,
    ...
    // Edge shimmer while the beam is held.
    ...
    strikeInMs: 350, strikeOutMs: 800, hitHoldMs: 0, impactHoldMs: 1600,
    ...
    // 0.45 viewport-heights, not 0.95: at 0.95 she starts further above the
    // frame than the frame is TALL, so most of the descent happened
    // off-screen and she appeared to blink in near the ground.
    descendFromFrac: 0.45, descendBeats: [0.46, 0.78, 1.0],
    descendDropMs: 420, descendReboundFrac: 0.035, descendReboundMs: 210,

    color: 0xff8a3d, coreColor: 0xfff0d0,
    spread: 'one',                      // 'one' | 'all'

    sfxZeroCutin: null, sfxZeroCutinVol: 0.9,
    ...
    sfxDescend: null, sfxDescendVol: 0.7,
    sfxLand: null, sfxLandVol: 0.85,
  ...
  const body = [
    ...
    "}",
    "",
    "/* ── P00  Zero Power cut-in ───────────────────────────────────────────── */",
    "// The house pattern, matching Rauben's Night of Hundred Bandits: dim, the",
    "// portrait in from the left, a hold, then out to the right. The picture",
    "// comes from the ACTOR -- system.props.cut_in_zero_power, falling back to",
    "// her sheet art -- so the sheet owns it and this script does not have to.",
    "const zpUrl = (function () {",
    "  try {",
    "    const ac = caster && caster.actor ? caster.actor : null;",
    "    if (!ac) return '';",
    "    return foundry.utils.getProperty(ac, 'system.props.cut_in_zero_power') || ac.img || '';",
    "  } catch (e) { return ''; }",
    "})();",
    "",
    "if (zpUrl) {",
    "  const zpDim = plate(0x000000, 88, above, true);",
    "  await oni.tween({ from: 0, to: 1, duration: cfg.zpDimMs, ease: E.outQuad,",
    "    onUpdate: (v) => { zpDim.alpha = v * cfg.zpDimTo; } });",
    "  playSfx('sfxZeroCutin', 'sfxZeroCutinVol');",
    "",
    "  // 0 = off the left edge, 1 = seated, 2 = gone off the right.",
    "  let zpT = 0;",
    "  let zpSpr = null;",
    "  try {",
    "    const ztex = await loadTexture(zpUrl);",
    "    if (ztex) {",
    "      zpSpr = new PIXI.Sprite(ztex);",
    "      zpSpr.anchor.set(0, 1);          // bottom-left, so it stands on the seat line",
    "      zpSpr.zIndex = 92;",
    "      above.addChild(zpSpr);",
    "      pinsTop.push(function () {",
    "        if (!zpSpr || zpSpr.destroyed) return;",
    "        // FIT inside a box, do not scale by height alone. Rauben's cut-in is a",
    "        // tall character portrait; Fafnir's sheet art is a wide full-body",
    "        // dragon, and 0.90 screen-heights of THAT is nearly a screen-width",
    "        // across. Whichever limit binds first wins.",
    "        const maxH = S.wLen(S.H) * cfg.zpPortraitH;",
    "        const maxW = S.wLen(S.W) * cfg.zpPortraitW;",
    "        zpSpr.scale.set(Math.min(maxH / Math.max(1, ztex.orig.height),",
    "                                 maxW / Math.max(1, ztex.orig.width)));",
    "        const seat = S.S2W(cfg.zpInsetX, cfg.zpBottomY);",
    "        const offL = seat.x - zpSpr.width - S.wLen(80);",
    "        const offR = S.S2W(1, cfg.zpBottomY).x + S.wLen(40);",
    "        const x = (zpT <= 1) ? (offL + (seat.x - offL) * zpT)",
    "                             : (seat.x + (offR - seat.x) * (zpT - 1));",
    "        zpSpr.position.set(x, seat.y);",
    "      });",
    "    }",
    "  } catch (e) { console.warn(TAG, 'cut-in portrait failed', zpUrl); }",
    "",
    "  await oni.tween({ from: 0, to: 1, duration: cfg.zpInMs, ease: E.outCubic,",
    "    onUpdate: (v) => { zpT = v; } });",
    "  await wait(cfg.zpHoldMs);",
    "  await oni.tween({ from: 1, to: 2, duration: cfg.zpOutMs, ease: E.inOutCubic,",
    "    onUpdate: (v) => { zpT = v; } });",
    "  try { if (zpSpr) zpSpr.destroy(); } catch (e) {}",
    "",
    "  await oni.tween({ from: 1, to: 0, duration: cfg.zpDimOutMs, ease: E.inOutQuad,",
    "    onUpdate: (v) => { zpDim.alpha = v * cfg.zpDimTo; } });",
    "  try { zpDim.destroy(); } catch (e) {}",
    "}",
    "",
    "/* ── P01  Startup ─────────────────────────────────────────────────────── */",
    "await oni.camera.focus({ point: home, zoom: cfg.camZoom, duration: cfg.camInMs });",
    ...
    "// Dust kicked off the ground she just left.",
    "if (A.smokeWebm) {",
    "  oni.webm(A.smokeWebm, { size: S.wLen(S.H * cfg.smokeSize), x: home.x, y: home.y,",
    "    parent: host, zIndex: 4 });",
    "}",
    ...
    "]);",
    "",
    "/* ── P05  Hold on the settled sky ─────────────────────────────────────── */",
    "// No strikes here any more. They fought the charge cue that follows: two",
    "// rumbles inside four seconds read as noise rather than as weather.",
    ...
    "let cutinRise = 0;",
    "// ONE driver for her pull-back: 0 at the jaw, 1 fully withdrawn. She starts",
    "// going the moment the breath fires instead of waiting for the dolly beat,",
    "// so the recoil and the travel are the same motion.",
    "let retreat = 0;",
    "const cutin = await imageSprite(A.cutinImg, 40);",
    "if (cutin) {",
    "  const ta = (cutin.texture.orig.width || 1) / (cutin.texture.orig.height || 1);",
    "  pins.push(function () {",
    "    const h = S.wLen(S.H) * cfg.cutinH * (1 - retreat * (1 - cfg.dollyCutinScale));",
    "    cutin.height = h; cutin.width = h * ta;",
    "    const yFrom = cfg.cutinY + cfg.cutinFromFrac;",
    "    const fx = cfg.cutinX - retreat * cfg.dollyDriftX;",
    "    const fy = (yFrom + (cfg.cutinY - yFrom) * cutinRise) - retreat * cfg.dollyDriftY;",
    "    const p = S.S2W(fx, fy);",
    ...
    "    // bolts stick OUT of the body rather than being contained by it. Each",
    "    // carries its OWN opacity and girth: a field drawn at one alpha reads as",
    ...
    "}",
    "",
    "function drawBreath(g, m, ux, uy, len, w0, w1, shimmer) {",
    "  const nx = -uy, ny = ux;",
    "  const now = performance.now();",
    "",
    "  // The body is a SEGMENTED band, not one quad: a four-point wedge has",
    "  // nothing to animate along its length, so its edge could never shimmer",
    "  // while the beam is held.",
    "  const edgeW = function (k, side) {",
    "    const t = k / cfg.edgeSteps;",
    "    const base = w0 + (w1 - w0) * t;",
    "    if (!shimmer) return { t: t, w: base };",
    "    const ph = k * 1.7 + side * 2.1 + now / 1000 * cfg.shimmerHz * Math.PI * 2;",
    "    return { t: t, w: base * (1 + Math.sin(ph) * cfg.shimmerDepth) };",
    "  };",
    "  const band = function (color, scale, alpha) {",
    "    g.beginFill(color, alpha);",
    "    for (let k = 0; k <= cfg.edgeSteps; k++) {",
    "      const e = edgeW(k, 1); const d = len * e.t; const w = e.w * scale;",
    "      const x = m.x + ux * d + nx * w, y = m.y + uy * d + ny * w;",
    "      if (k === 0) { g.moveTo(x, y); } else { g.lineTo(x, y); }",
    "    }",
    "    for (let k = cfg.edgeSteps; k >= 0; k--) {",
    "      const e = edgeW(k, -1); const d = len * e.t; const w = e.w * scale;",
    "      g.lineTo(m.x + ux * d - nx * w, m.y + uy * d - ny * w);",
    "    }",
    "    g.closePath(); g.endFill();",
    "  };",
    "  band(cfg.color, 1, 0.55);",
    "  band(cfg.coreColor, 0.45, 0.95);",
    "",
    "  // Arcs are FILLED tapered slivers, not stroked polylines. A constant-width",
    "  // stroke with round caps read as soft noodles laid over the beam; these",
    "  // come to a point, have hard mitred edges, and each carries its own alpha.",
    "  if (now - lastArc > cfg.arcChurnMs) { lastArc = now; rollArcs(); }",
    "  const aw = S.wLen(S.H * cfg.arcWidth);",
    "  for (const a of arcs) {",
    ...
    "      pos.push({ x: m.x + ux * d + nx * p.j * half, y: m.y + uy * d + ny * p.j * half,",
    "                 w: aw * a.w * Math.pow(1 - k / (n - 1), cfg.arcTaper) });",
    "    }",
    "    // Offset along the LOCAL segment normal, not the beam normal. Offsetting",
    "    // a zig-zagging centreline by a fixed direction makes the ribbon cross",
    "    // itself, and PIXI fills the crossing as a hollow angular blob.",
    "    const pn = [];",
    "    for (let k = 0; k < n; k++) {",
    "      const q0 = pos[Math.max(0, k - 1)], q1 = pos[Math.min(n - 1, k + 1)];",
    "      let dx = q1.x - q0.x, dy = q1.y - q0.y;",
    "      const dl = Math.hypot(dx, dy) || 1;",
    "      pn.push({ x: -dy / dl, y: dx / dl });",
    "    }",
    "    g.beginFill(cfg.coreColor, a.alpha);",
    "    g.moveTo(pos[0].x + pn[0].x * pos[0].w, pos[0].y + pn[0].y * pos[0].w);",
    "    for (let k = 1; k < n; k++) { g.lineTo(pos[k].x + pn[k].x * pos[k].w, pos[k].y + pn[k].y * pos[k].w); }",
    "    for (let k = n - 1; k >= 0; k--) { g.lineTo(pos[k].x - pn[k].x * pos[k].w, pos[k].y - pn[k].y * pos[k].w); }",
    "    g.closePath(); g.endFill();",
    "  }",
    "}",
    "",
    ...
    "    const gs = 1 + retreat * (cfg.dollyBeamScale - 1);",
    "    const w0 = S.wLen(S.H * cfg.beamWidthNear) * gs;",
    "    const w1 = S.wLen(S.H * cfg.beamWidthFar) * gs;",
    "    drawBreath(breath, m, ux, uy, len, w0, w1, true);",
    "  });",
    "}",
    "",
    ...
    "  retreat = v * cfg.retreatOnFire;",
    "  if (ballSpr) ballSpr.alpha = Math.min(1, v * 2);",
    "  charge.alpha = 1 - v * 0.6;",
    "} });",
    "",
    "// The hold is not a freeze -- she keeps drifting back through it, which is",
    "// what stops the held beam reading as a still frame.",
    "await oni.tween({ from: 0, to: 1, duration: cfg.beamHoldMs, ease: E.linear, onUpdate: (v) => {",
    "  retreat = cfg.retreatOnFire + (cfg.retreatOnHold - cfg.retreatOnFire) * v;",
    "  ballT = 0.25 + v * 0.10;",
    "} });",
    "",
    "/* ── P10  Ride it out ─────────────────────────────────────────────────── */",
    ...
    "// Linear, not eased: an inOut curve spent its last third decelerating, so",
    "// she drifted to a stop exactly when she should have been travelling.",
    "const dollying = oni.tween({ from: 0, to: 1, duration: cfg.dollyMs, ease: E.linear, onUpdate: (v) => {",
    "  retreat = cfg.retreatOnHold + (1 - cfg.retreatOnHold) * v;",
    "  ballT = 0.35 + v * 0.65;",
    "  charge.alpha = 0.4 * (1 - v);",
    "} });",
    "",
    "/* ── P11 + P12  Whiteout, taken MID-MOVE ──────────────────────────────── */",
    "// The white starts partway INTO the dolly and is opaque before it ends, so",
    "// the frame is taken while she is still travelling. Waiting for the retreat",
    "// to finish first left the cut-in visibly parked for a beat, which reads as",
    "// the shot running out rather than being cut off at its peak.",
    "const white = plate(0xffffff, 90, above, true);",
    "await wait(cfg.whiteDelayMs);",
    ...
    "    drawBreath(hit, m, ux2, uy2, reach * 2.2, w * 0.55, w, true);",
    "  } else {",
    "    for (const v of victims) {",
    "      const c = ctr(v);",
    "      const m = { x: c.x - ux2 * reach, y: c.y - uy2 * reach };",
    "      const w = S.wLen(S.H * cfg.strikeWidth) * hitGain;",
    "      drawBreath(hit, m, ux2, uy2, reach, w * 0.5, w, true);",
    "    }",
    "  }",
    "};",
    "pinsTop.push(drawHit);",
    "",
    "// Fire's blow ARRIVES rather than being drawn: the same fireball she",
    "// breathed falls in from off-frame top-left onto the victim. A beam wearing",
    "// fire colours is the one thing this branch was not supposed to be.",
    "if (cfg.impactStyle === 'ball') {",
    "  const landAt = victims.length ? ctr(victims[0]) : cen;",
    "  const from = S.S2W(cfg.ballFromX, cfg.ballFromY);",
    "  const fb = await oni.webm(A.ballWebm, { size: S.wLen(S.H * cfg.ballImpactSize),",
    "    x: from.x, y: from.y, parent: strike, loop: true, zIndex: 20,",
    "    blend: PIXI.BLEND_MODES.ADD, alpha: 1 });",
    "  const fbs = (fb && fb.sprite) ? fb.sprite : null;",
    "  const fbBase = fbs ? fbs.scale.x : 1;",
    "  await oni.tween({ from: 0, to: 1, duration: cfg.ballFallMs, ease: E.inQuad, onUpdate: (v) => {",
    "    if (!fbs) return;",
    "    fbs.position.set(from.x + (landAt.x - from.x) * v, from.y + (landAt.y - from.y) * v);",
    "    fbs.scale.set(fbBase * (1 + v * (cfg.ballImpactGrow - 1)));",
    "  } });",
    "  try { if (fbs) fbs.destroy(); } catch (e) {}",
    "}",
    "",
    "playSfx('sfxImpact', 'sfxImpactVol');",
    ...
    "done();",
    "",
    "// The jolt starts on the SAME frame the blow lands. It is not awaited, so",
    "// holding before it would simply delay the recoil by the length of the hold.",
    "const shaking = Promise.all(victims.map((v) => shakeTarget(v, cfg.shakeMs, S.wLen(cfg.shakeAmp))));",
    "",
    "// Let the blow sit before the frame is taken. Without this the bolt was on",
    "// screen for barely a third of a second before the white swallowed it.",
    "await wait(cfg.hitHoldMs);",
    "",
    ...
    "",
    "// Ball mode never DREW a hit beam, so fading one from 1 would create one:",
    "// the tween writes hitGain = 1 on its first frame, and a beam nobody asked",
    "// for eased out over the fireball that had already landed.",
    "const fading = (cfg.impactStyle === 'ball')",
    "  ? Promise.resolve()",
    "  : oni.tween({ from: 1, to: 0, duration: cfg.strikeOutMs, ease: E.inOutQuad,",
    "      onUpdate: (v) => { hitGain = v; } });",
    ...
    "  playSfx('sfxDescend', 'sfxDescendVol');",
    ...
    "    curY = toY;",
    "  }",
    "  // She lands. The thump is the last thing the sequence does.",
    "  if (A.smokeWebm) {",
    "    oni.webm(A.smokeWebm, { size: S.wLen(S.H * cfg.smokeSize), x: home.x, y: home.y,",
    "      parent: above, zIndex: 78 });",
    "  }",
    "  playSfx('sfxLand', 'sfxLandVol');",
    "  oni.screenshake({ duration: cfg.shakeLandMs, intensity: cfg.shakeLand });",
    "}",
    "restoreCaster();",
    "try { if (clone) clone.destroy(); } catch (e) {}",
