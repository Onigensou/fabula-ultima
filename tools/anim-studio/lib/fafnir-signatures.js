...
function torment(opts = {}) {
  const cfg = Object.assign({
    ...
    // The draconic eyes, the same pair Draconic Domination used to open with.
    // Placement is in SCREEN fractions: the camera is centred on the victim by
    // this point, so screen space puts the pair symmetrically around them
    // whatever the board layout is. Domination stares and holds; here the pair
    // sinks INTO them, which is the difference between being dominated and
    // being tormented.
    ...
    eyeColor: 0xff2f4a, glowColor: 0xff3324,
    squashFrom: 0.16,
    glowHz: 1.6, glowDepth: 0.22,
    // riseFrac is gone with the drawn mask: it seated the face ABOVE the
    // token, and the eyes are placed in screen space instead.
  ...
  const body = [
    ...
    "await wait(cfg.beatMs);",
    "",
    "// The draconic eyes, anchored in SCREEN space around the victim, who the",
    "// camera has centred.",
    ...
    "  // NEGATIVE sign: screen space has y DOWN, so this is the rotation that",
    "  // puts both OUTER ends high and converges the inner ends down onto the",
    "  // victim between them.",
    ...
    "  c.alpha = 0;",
    "  host.addChild(c);",
    "  return { c, glow, at, tilt: -sign * cfg.eyeTilt };",
    "}",
    ...
    "  for (const e of eyes) { e.c.scale.set(1, sy); e.c.alpha = Math.min(1, v * 2.2); }",
    "} });",
    "",
    "// Leer — they breathe while the laugh plays out.",
    "const leer0 = performance.now();",
    "await oni.tween({ from: 0, to: 1, duration: cfg.leerMs, ease: E.linear, onUpdate: () => {",
    "  const sw = Math.sin((performance.now() - leer0) / 1000 * cfg.glowHz * Math.PI * 2);",
    "  const k = 1 + sw * cfg.glowDepth;",
    "  for (const e of eyes) {",
    "    e.glow.width = HW * 2.6 * k;",
    "    e.glow.height = HH * 3.4 * k;",
    "    e.glow.alpha = 0.75 + sw * 0.25;",
    "  }",
    "} });",
    "",
    "// Sink INTO them: the pair converges on the victim and shrinks, not just",
    "// fades. Domination holds the stare and then simply shuts — this one goes",
    "// in, which is the whole difference between the two actions.",
    "await oni.tween({ from: 0, to: 1, duration: cfg.descendMs, ease: E.inOutQuad, onUpdate: (v) => {",
    "  const k = 1 - v * 0.82;",
    "  for (const e of eyes) {",
    "    e.c.position.set(e.at.x + (t.x - e.at.x) * v, e.at.y + (t.y - e.at.y) * v);",
    "    e.c.scale.set(k, k);",
    "    // Level out as they converge, so they read as going in rather than",
    "    // merely sliding.",
    "    e.c.rotation = e.tilt * (1 - v * 0.6);",
    "    e.c.alpha = 1 - v * 0.85;",
    "  }",
    "} });",
