...
function draconicDomination(opts = {}) {
  const cfg = Object.assign({
    camZoom: 1.7, camInMs: 1200, camOutMs: 1100,
    ...
    // Eye placement, in SCREEN fractions. The camera is centred on the victim
    // by this point, so screen space puts the pair symmetrically around them
    // whatever the board layout is.
    eyeY: 0.36, eyeSpread: 0.25,
    ...
    lineCount: 132, lineColor: 0x140704, lineAlpha: 0.9,
    lineLenMin: 0.06, lineLenMax: 0.30,   // fraction of viewport height
  ...
  const body = [
    ...
    "",
    "// ── Speedlines ──",
    "// Slivers driven in from the rim of the frame. Re-cast on a timer rather",
    "// than tweened, because the violence is in the field CHANGING — a fixed set",
    "// of lines just reads as a decorative border.",
    ...
    "    // Pick a point on the rim and drive inward along the screen normal.",
    "    const side = i % 4;",
    "    const u = Math.random();",
    "    let fx, fy, nx, ny;",
    "    if (side === 0) { fx = u; fy = 0; nx = 0; ny = 1; }",
    "    else if (side === 1) { fx = 1; fy = u; nx = -1; ny = 0; }",
    "    else if (side === 2) { fx = u; fy = 1; nx = 0; ny = -1; }",
    "    else { fx = 0; fy = u; nx = 1; ny = 0; }",
    "    const len = (cfg.lineLenMin + Math.random() * (cfg.lineLenMax - cfg.lineLenMin)) * lineGain;",
    ...
    "    const tipF = { x: fx + nx * len * (H / W), y: fy + ny * len };",
    "    const aF = { x: fx - ny * halfW * (H / W), y: fy + nx * halfW };",
    "    const bF = { x: fx + ny * halfW * (H / W), y: fy - nx * halfW };",
    ...
    "",
    "// ── The eyes ──",
    "// Anchored in SCREEN space around the victim, who the camera has centred.",
    "function lens(g, hw, hh, color, alpha) {",
    "  g.beginFill(color, alpha);",
    "  g.moveTo(-hw, 0);",
    "  g.quadraticCurveTo(0, -hh, hw, 0);",
    "  g.quadraticCurveTo(0, hh, -hw, 0);",
    "  g.endFill();",
    "  g.blendMode = PIXI.BLEND_MODES.ADD;",
    "}",
    "const HW = S.hPx(cfg.eyeHalfW * (S.W / S.H));",
    "const HH = S.hPx(cfg.eyeHalfH);",
    "const glowTex = oni.radialTexture('rgba(255,255,255,1)', 'rgba(255,255,255,0)', { size: 128 });",
    "",
    "function buildEye(sign) {",
    "  const at = S.S2W(0.5 + sign * cfg.eyeSpread, cfg.eyeY);",
    "  const c = new PIXI.Container();",
    "  c.position.set(at.x, at.y);",
    "  // NEGATIVE sign. Screen space has y DOWN, so a positive rotation drives",
    "  // the +x end of the lens downward: sign * tilt gives the left eye a slash",
    "  // and the right a backslash, which is the pair splayed outward-down. The",
    "  // reference has both OUTER ends high and the inner ends converging down",
    "  // onto the victim between them, which is the opposite rotation.",
    "  c.rotation = -sign * cfg.eyeTilt;",
    "  const glow = new PIXI.Sprite(glowTex);",
    "  glow.anchor.set(0.5);",
    "  glow.tint = cfg.glowColor;",
    ...
    "// They OPEN: squashed flat, then stretched vertically to full height.",
    "await oni.tween({ from: 0, to: 1, duration: cfg.openMs, ease: E.outQuad, onUpdate: function (v) {",
    "  const sy = cfg.squashFrom + (1 - cfg.squashFrom) * v;",
    ...
    "done();",
    "",
    "// ── Idle hold: the eyes breathe, the lines keep tearing ──",
    ...
    "  const k = 1 + s * cfg.glowDepth;",
    "  for (const e of [eyeL, eyeR]) {",
    "    e.glow.width = HW * 2.6 * k;",
    ...
    "// shot was built in, run backwards.",
    "await oni.tween({ from: cfg.bystanderTo, to: 1, duration: cfg.camOutMs, ease: E.inOutQuad,",
    "  onUpdate: function (v) { hold = v; } });",
    "restoreOthers();",
    "await oni.camera.restore({ duration: cfg.camOutMs });",
    "await dim.fadeOut({ duration: cfg.camOutMs });",
  ].join("\n");
