...
/* ── Kirin · Rail Stream ─────────────────────────────────────────────────── */
//
// Everything the Kirin has been hoarding leaves the horn at once: dim, pull
// back to take in the field, gather blue charge, fire a column straight up out
// of frame, and bring it back down as a storm.
//
// The falling bolts are drawn, not an asset. A video clip was tried and
// abandoned: a clip is placed at full length with no leading edge, so however
// the spawn times are scheduled the strikes cannot read as sequential. Drawing
// them means each one can grow, flicker and forkits own way, which is what
// makes a storm look like a storm.
//
// Each bolt is a jagged polyline redrawn every few frames (the flicker), in
// three stacked passes — a wide translucent aura, a softer body, a thin white
// core — with the aura pulsing, and a particle burst where it earths.

function railStream(opts = {}) {
  const cfg = Object.assign({
    // <1 pulls OUT (zoom is a multiple of rest scale): this shot wants the whole
    // battlefield, not the Kirin.
    ...
    // Bolt shape. jag is the horizontal wander per segment; segments is how many
    // kinks. flickerMs is how often the whole path is re-rolled — this is the
    // single biggest lever on how violent it reads, so it is deliberately near
    // frame rate. whip snaps the whole bolt sideways between re-rolls.
    rainSegments: 14, rainJag: 42, rainFlickerMs: 22, rainWhip: 26,
    rainCoreWidth: 5, rainBodyWidth: 17, rainAuraWidth: 48,
    rainPulseHz: 16, rainForkChance: 0.7, rainForkLen: 0.32,
    // Per-bolt opacity spread, so a storm has faint strikes and blazing ones
    // rather than thirty identical bolts.
    rainAlphaMin: 0.45, rainAlphaMax: 1,
    // Chance a strike is answered immediately by a second one beside it.
    rainTwinChance: 0.3, rainTwinGapMs: 70, rainTwinSpread: 0.05,
    // Slot jitter. Stays under 1 slot so the window is always covered — a raw
    // random draw leaves dead stretches and dumps the rest together.
    rainChaos: 0.92,
    // Earthing burst
    groundCount: 22, groundRadius: 120, groundSize: 13,

    // Per-target strikes (same renderer, heavier)
    hitLeadMs: 420, hitGapMs: 190,
    hitSegments: 16, hitJag: 46, hitCoreWidth: 8, hitBodyWidth: 26, hitAuraWidth: 70,
  ...
  const body = [
    ...
    "",
    "// ── lightning renderer ────────────────────────────────────────────────",
    "// A bolt is a jagged path re-rolled on a timer, stroked three times: wide",
    "// translucent aura, softer body, thin white core. Stacking additive strokes",
    "// is what gives it a glowing translucent look rather than a flat line.",
    "function jaggedPath(x, y0, y1, segs, jag) {",
    "  const pts = [];",
    "  for (let i = 0; i <= segs; i++) {",
    "    const t = i / segs;",
    "    const off = (i === 0 || i === segs) ? 0 : (Math.random() * 2 - 1) * jag;",
    "    pts.push({ x: x + off, y: y0 + (y1 - y0) * t });",
    "  }",
    "  return pts;",
    "}",
    "function strokePath(g, pts, width, color, alpha) {",
    "  if (pts.length < 2 || alpha <= 0 || width <= 0) return;",
    "  g.lineStyle({ width: width, color: color, alpha: alpha, cap: 'round', join: 'round' });",
    "  g.moveTo(pts[0].x, pts[0].y);",
    "  for (let i = 1; i < pts.length; i++) g.lineTo(pts[i].x, pts[i].y);",
    "}",
    "function drawBolt(g, pts, w, pulse, alpha) {",
    "  g.clear();",
    "  strokePath(g, pts, S.wLen(w.aura) * pulse, cfg.auraColor, 0.16 * alpha * pulse);",
    "  strokePath(g, pts, S.wLen(w.body) * (0.85 + 0.15 * pulse), cfg.color, 0.5 * alpha);",
    "  strokePath(g, pts, S.wLen(w.core), cfg.coreColor, alpha);",
    "  // Forks: short branches off a random kink, so no two strikes are alike.",
    "  if (w.fork && pts.length > 3) {",
    "    for (let i = 2; i < pts.length - 1; i++) {",
    "      if (Math.random() > cfg.rainForkChance / pts.length) continue;",
    "      const p = pts[i];",
    "      const span = (pts[pts.length - 1].y - pts[0].y) * cfg.rainForkLen;",
    "      const branch = jaggedPath(p.x, p.y, p.y + span, 3, S.wLen(cfg.rainJag) * 1.4);",
    "      for (const q of branch) q.x += (Math.random() * 2 - 1) * S.wLen(cfg.rainJag);",
    "      strokePath(g, branch, S.wLen(w.body) * 0.5, cfg.color, 0.4 * alpha);",
    "      strokePath(g, branch, S.wLen(w.core) * 0.5, cfg.coreColor, 0.8 * alpha);",
    "    }",
    "  }",
    "}",
    "",
    "// One strike: grows to full extent, crackles in place, earths, fades.",
    "async function strike(x, yTop, yBot, w, opt) {",
    "  opt = opt || {};",
    "  const g = new PIXI.Graphics();",
    "  g.blendMode = PIXI.BLEND_MODES.ADD;",
    "  host.addChild(g);",
    "  // Every strike gets its own opacity and its own girth, so the storm is not",
    "  // thirty copies of one bolt.",
    "  const A = opt.alpha == null",
    "    ? cfg.rainAlphaMin + Math.random() * (cfg.rainAlphaMax - cfg.rainAlphaMin)",
    "    : opt.alpha;",
    "  const gir = opt.girth == null ? (0.75 + Math.random() * 0.6) : opt.girth;",
    "  const W = { segs: w.segs, jag: w.jag, fork: w.fork,",
    "    core: w.core * gir, body: w.body * gir, aura: w.aura * gir };",
    "  let bx = x;",
    "  let pts = jaggedPath(bx, yTop, yTop, W.segs, S.wLen(W.jag));",
    "  let lastRoll = 0;",
    "  let earthed = false;",
    "  const fall = opt.fallMs == null ? cfg.rainFallMs : opt.fallMs;",
    "  await oni.tween({ from: 0, to: 1, duration: fall, ease: E.inQuad, onUpdate: (v) => {",
    "    const reach = Math.min(1, v * 1.6);",
    "    const y = yTop + (yBot - yTop) * reach;",
    "    const now = performance.now();",
    "    if (now - lastRoll >= cfg.rainFlickerMs) {",
    "      lastRoll = now;",
    "      // Snap the whole bolt sideways as well as re-rolling its kinks — a",
    "      // path that only wiggles internally still reads as a stationary line.",
    "      bx = x + (Math.random() * 2 - 1) * S.wLen(cfg.rainWhip);",
    "      pts = jaggedPath(bx, yTop, y, W.segs, S.wLen(W.jag));",
    "    } else { const last = pts[pts.length - 1]; if (last) last.y = y; }",
    "    const pulse = 0.65 + 0.35 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz);",
    "    drawBolt(g, pts, W, pulse, A);",
    "    if (!earthed && reach >= 1) {",
    "      earthed = true;",
    "      oni.particles({ x: x, y: yBot, count: opt.groundCount || cfg.groundCount, color: cfg.color,",
    "        size: cfg.groundSize, radius: S.wLen(opt.groundRadius || cfg.groundRadius), life: 700,",
    "        gravity: S.wLen(-40), blend: PIXI.BLEND_MODES.ADD, parent: host });",
    "      if (opt.onEarth) { try { opt.onEarth(); } catch (e) {} }",
    "    }",
    "  } });",
    "  // Crackle in place before dying — a bolt that vanishes the instant it lands",
    "  // reads as a line being deleted rather than lightning discharging.",
    "  const hold = opt.holdMs == null ? cfg.rainHoldMs : opt.holdMs;",
    "  await oni.tween({ from: 0, to: 1, duration: hold, ease: E.linear, onUpdate: (v) => {",
    "    const now = performance.now();",
    "    if (now - lastRoll >= cfg.rainFlickerMs) {",
    "      lastRoll = now;",
    "      bx = x + (Math.random() * 2 - 1) * S.wLen(cfg.rainWhip);",
    "      pts = jaggedPath(bx, yTop, yBot, W.segs, S.wLen(W.jag));",
    "    }",
    "    drawBolt(g, pts, W, 0.65 + 0.35 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz), A);",
    "  } });",
    "  await oni.tween({ from: 1, to: 0, duration: cfg.rainFadeMs, ease: E.inQuad, onUpdate: (a) => {",
    "    drawBolt(g, pts, W, 0.7, a * A);",
    "  } });",
    ...
    "",
    "// The column: straight up out of frame, in screen fractions so it always",
    "// clears the top edge whatever the zoom.",
    ...
    "  const pulse = 0.7 + 0.3 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz);",
    "  const pts = jaggedPath(home.x, home.y, y, cfg.rainSegments, S.wLen(cfg.rainJag * 0.6));",
    "  drawBolt(col, pts, { aura: cfg.columnWidth, body: cfg.columnWidth * 0.45, core: cfg.columnWidth * 0.18, segs: cfg.rainSegments, jag: cfg.rainJag, fork: false }, pulse, 1);",
    "} });",
    ...
    "const perBolt = cfg.rainWindowMs / Math.max(1, cfg.rainCount);",
    "const rainW = { segs: cfg.rainSegments, jag: cfg.rainJag, core: cfg.rainCoreWidth,",
    "  body: cfg.rainBodyWidth, aura: cfg.rainAuraWidth, fork: true };",
    "const rain = [];",
    "// Jitter each bolt around its slot rather than firing on a metronome. The",
    "// jitter is kept under one slot on purpose: a free random draw leaves dead",
    "// stretches and dumps the remainder together, which reads as one flash.",
    "for (let i = 0; i < cfg.rainCount; i++) {",
    "  const fx = 0.04 + Math.random() * 0.92;",
    "  const top = S.S2W(fx, -0.1);",
    "  const bot = S.S2W(fx, 0.7 + Math.random() * 0.35);",
    "  const at = Math.max(0, (i + (Math.random() * 2 - 1) * cfg.rainChaos) * perBolt);",
    "  rain.push(wait(at).then(() => strike(top.x, top.y, bot.y, rainW, {",
    "    fallMs: cfg.rainFallMs * (0.7 + Math.random() * 0.7),",
    "  })));",
    "  // Twin strike: a second bolt right beside the first, a beat later. Real",
    "  // storms double up, and it breaks any residual regularity in the rhythm.",
    "  if (Math.random() < cfg.rainTwinChance) {",
    "    const fx2 = Math.min(0.98, Math.max(0.02, fx + (Math.random() * 2 - 1) * cfg.rainTwinSpread));",
    "    const t2 = S.S2W(fx2, -0.1);",
    "    const b2 = S.S2W(fx2, 0.7 + Math.random() * 0.35);",
    "    rain.push(wait(at + cfg.rainTwinGapMs * (0.5 + Math.random())).then(() => strike(t2.x, t2.y, b2.y, rainW, {",
    "      fallMs: cfg.rainFallMs * (0.7 + Math.random() * 0.6),",
    "    })));",
    "  }",
    "}",
    "",
    "// Every target actually gets hit, inside the same window.",
    "const hitW = { segs: cfg.hitSegments, jag: cfg.hitJag, core: cfg.hitCoreWidth,",
    "  body: cfg.hitBodyWidth, aura: cfg.hitAuraWidth, fork: true };",
    "const hits = [];",
    "for (let i = 0; i < tgts.length; i++) {",
    "  const tk = tgts[i];",
    "  const c = ctr(tk);",
    "  const top = S.S2W(0.5, -0.1);",
    "  hits.push(wait(cfg.hitLeadMs + i * cfg.hitGapMs).then(() => strike(c.x, top.y, c.y, hitW, {",
    "    alpha: 1, girth: 1.15,",
    "    groundCount: cfg.impactCount, groundRadius: cfg.impactRadius, holdMs: cfg.rainHoldMs * 1.4,",
    "    onEarth: () => {",
    "      playSfx('sfxImpact', 'sfxImpactVol');",
    "      oni.screenshake({ duration: 420, intensity: cfg.screenshakeAmp + 3 });",
...
function stinkyBreath(opts = {}) {
  const cfg = Object.assign({
    ...
    // Pull OUT and stay there. No push-in on the caster and no whip to the
    // party — the shot is about the whole field being covered, and two extra
