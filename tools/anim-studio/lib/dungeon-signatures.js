...
function railStream(opts = {}) {
  const cfg = Object.assign({
    ...
    rainPulseHz: 16,
    // Crackle: small, low-opacity copies of the lightning asset dotted along
    // the beam. Deliberately faint — they are texture around the beam, not the
    // strike itself.
    crackleWebm: null, crackleChance: 0.85, crackleCount: 3,
    crackleAlpha: 0.3, crackleSize: 0.3, crackleThickness: 0.34,
    crackleSpread: 46, crackleAngleJitter: 34,
  ...
  const body = [
    ...
    "",
    "// ── beam renderer ─────────────────────────────────────────────────────",
    "// A straight vertical beam, stroked three times: wide translucent aura,",
    "// softer body, thin white core. Stacking additive strokes is what gives it",
    "// a glowing translucent look rather than a flat line. The aura pulses.",
    "function drawBeam(g, x, y0, y1, w, pulse, alpha) {",
    "  g.clear();",
    "  const stroke = (width, color, a) => {",
    "    if (a <= 0 || width <= 0) return;",
    "    g.lineStyle({ width: width, color: color, alpha: a, cap: 'round' });",
    "    g.moveTo(x, y0); g.lineTo(x, y1);",
    "  };",
    "  stroke(S.wLen(w.aura) * pulse, cfg.auraColor, 0.16 * alpha * pulse);",
    "  stroke(S.wLen(w.body) * (0.85 + 0.15 * pulse), cfg.color, 0.5 * alpha);",
    "  stroke(S.wLen(w.core), cfg.coreColor, alpha);",
    "}",
    "",
    "// Scatter small, faint copies of the real lightning clip along the beam.",
    "// This is where the natural crackle comes from — a drawn zig-zag never",
    "// stops looking hand-made. restart:false because Foundry caches the video",
    "// texture by URL, so every sprite shares one <video>; restarting it would",
    "// snap them all to frame 0 together.",
    "async function crackle(x, y0, y1, n, alphaMul) {",
    "  if (!cfg.crackleWebm || n <= 0) return;",
    "  const span = y1 - y0;",
    "  for (let i = 0; i < n; i++) {",
    "    const t = (i + 0.5) / n + (Math.random() * 0.3 - 0.15);",
    "    const len = Math.abs(span) * cfg.crackleSize * (0.6 + Math.random() * 0.8);",
    "    const cx = x + (Math.random() * 2 - 1) * S.wLen(cfg.crackleSpread);",
    "    const cy = y0 + span * Math.max(0.05, Math.min(0.95, t));",
    "    const r = await oni.webm(cfg.crackleWebm, { size: len, x: cx, y: cy,",
    "      parent: host, zIndex: 94500, restart: false });",
    "    if (!r || !r.sprite) continue;",
    "    const sp = r.sprite;",
    "    sp.anchor.set(0, 0.5);",
    "    sp.width = len;",
    "    sp.height = len * cfg.crackleThickness;",
    "    sp.position.set(cx, cy);",
    "    // Roughly along the beam, jittered so the shared frame is not obvious.",
    "    sp.rotation = Math.PI / 2 + (Math.random() * 2 - 1) * (cfg.crackleAngleJitter * Math.PI / 180);",
    "    sp.alpha = cfg.crackleAlpha * alphaMul * (0.6 + Math.random() * 0.7);",
    ...
    "  const W = { core: w.core * gir, body: w.body * gir, aura: w.aura * gir };",
    "  let earthed = false;",
    "  let crackled = false;",
    ...
    "    drawBeam(g, x, yTop, y, W, pulse, A);",
    "    if (!crackled && reach > 0.35) {",
    "      crackled = true;",
    "      if (Math.random() < (opt.crackleChance == null ? cfg.crackleChance : opt.crackleChance)) {",
    "        crackle(x, yTop, yBot, opt.crackleCount == null ? cfg.crackleCount : opt.crackleCount, A);",
    "      }",
    "    }",
    "    if (!earthed && reach >= 1) {",
    "      earthed = true;",
    ...
    "    drawBeam(g, x, yTop, yBot, W, 0.65 + 0.35 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz), A);",
    "  } });",
    "  await oni.tween({ from: 1, to: 0, duration: cfg.rainFadeMs, ease: E.inQuad, onUpdate: (a) => {",
    "    drawBeam(g, x, yTop, yBot, W, 0.7, a * A);",
    "  } });",
    ...
    "  drawBeam(col, home.x, home.y, y, { aura: cfg.columnWidth, body: cfg.columnWidth * 0.45, core: cfg.columnWidth * 0.18 }, pulse, 1);",
    "} });",
    ...
    "const rainW = { core: cfg.rainCoreWidth, body: cfg.rainBodyWidth, aura: cfg.rainAuraWidth };",
    "const rain = [];",
    ...
    "const hitW = { core: cfg.hitCoreWidth, body: cfg.hitBodyWidth, aura: cfg.hitAuraWidth };",
    "const hits = [];",
    "for (let i = 0; i < tgts.length; i++) {",
    "  const tk = tgts[i];",
    "  const c = ctr(tk);",
    ...
    "    alpha: 1, girth: 1.15, crackleChance: 1, crackleCount: cfg.hitCrackleCount,",
