...
function railStream(opts = {}) {
  ...
  const body = [
    ...
    "}",
    "",
    "// Jagged companion strands, drawn faint and re-rolled constantly. These sit",
    "// BEHIND the beam: the straight beam is the strike and stays readable,",
    "// while these supply the restless crackle a bare line cannot.",
    ...
    "}",
    "function drawCompanions(g, x, y0, y1, n, alpha, pulse) {",
    "  g.clear();",
    "  if (n <= 0 || alpha <= 0) return;",
    "  for (let i = 0; i < n; i++) {",
    "    const pts = jaggedPath(x, y0, y1, cfg.jagSegments, S.wLen(cfg.jagSpread));",
    "    strokePath(g, pts, S.wLen(cfg.jagBodyWidth) * pulse, cfg.color, 0.55 * alpha);",
    "    strokePath(g, pts, S.wLen(cfg.jagCoreWidth), cfg.coreColor, alpha);",
    "  }",
    "}",
    "",
    ...
    "  opt = opt || {};",
    "  // Two layers: companions behind, beam in front.",
    "  host.sortableChildren = true;",
    "  const gj = new PIXI.Graphics();",
    "  gj.blendMode = PIXI.BLEND_MODES.ADD;",
    "  gj.zIndex = 1;",
    "  host.addChild(gj);",
    ...
    "  let earthed = false;",
    "  let lastJag = 0;",
    "  const nJag = opt.jagCount == null ? cfg.jagCount : opt.jagCount;",
    ...
    "    if (now - lastJag >= cfg.jagFlickerMs) {",
    "      lastJag = now;",
    "      drawCompanions(gj, x, yTop, y, nJag, cfg.jagAlpha * A, pulse);",
    "    }",
    "    if (!earthed && reach >= 1) {",
    "      earthed = true;",
    ...
    "    const pulse = 0.65 + 0.35 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz);",
    "    drawBeam(g, x, yTop, yBot, W, pulse, A);",
    "    const now = performance.now();",
    "    if (now - lastJag >= cfg.jagFlickerMs) {",
    "      lastJag = now;",
    "      drawCompanions(gj, x, yTop, yBot, nJag, cfg.jagAlpha * A, pulse);",
    "    }",
    "  } });",
    "  await oni.tween({ from: 1, to: 0, duration: cfg.rainFadeMs, ease: E.inQuad, onUpdate: (a) => {",
    "    drawBeam(g, x, yTop, yBot, W, 0.7, a * A);",
    "    drawCompanions(gj, x, yTop, yBot, nJag, cfg.jagAlpha * A * a, 0.7);",
    "  } });",
    "  try { g.destroy(); gj.destroy(); } catch (e) {}",
    "}",
    "",
    "// ── wind-up ───────────────────────────────────────────────────────────",
    "// Pull back on the STAGE CENTRE. Aimed at the caster, a zoom-out becomes a",
    "// pan toward the Kirin instead of a reveal of the arena.",
    "const camming = oni.camera.focus({ point: stageCentre(), zoom: cfg.camZoom, duration: cfg.camInMs });",
    "const dimmer = await oni.sceneDim({ to: cfg.dimTo, fadeIn: cfg.dimMs });",
    "await camming;",
    "",
    "playSfx('sfxCharge', 'sfxChargeVol');",
    ...
    "    alpha: 1, girth: 1.15, jagCount: cfg.hitJagCount,",
...
function stinkyBreath(opts = {}) {
  ...
  const body = [
    ...
    "",
    "const dimmer = await oni.sceneDim({ to: cfg.dimTo, fadeIn: cfg.dimMs });",
    "// Pull back on the STAGE CENTRE and hold there.",
    "await oni.camera.focus({ point: stageCentre(), zoom: cfg.camZoomIn, duration: cfg.camInMs });",
    "",
