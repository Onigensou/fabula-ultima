...
function railStream(opts = {}) {
  ...
  const body = [
    ...
    "// Each bolt gets an even SLOT, then jitters around it. A raw random draw",
    "// (and especially a power curve, which biases toward 1) leaves whole",
    "// stretches empty and dumps the rest together — which reads as one sheet",
    "// falling, not a storm. Slot+jitter guarantees the window is covered while",
    "// still landing irregularly.",
    "const perBolt = cfg.rainWindowMs / Math.max(1, cfg.rainCount);",
    "const boltDelay = (i) => Math.max(0, (i + (Math.random() * 2 - 1) * cfg.rainChaos) * perBolt);",
    "const rain = [];",
    ...
    "      const fall = Math.abs(bot.y - top.y) * cfg.rainWebmScale * jit;",
    "      const thick = S.hPx(cfg.rainWebmThickness) * jit;",
    "      const r = await oni.webm(cfg.rainWebm, { size: fall, x: top.x, y: top.y,",
    "        parent: host, zIndex: 95000 });",
    "      if (!r || !r.sprite) return null;",
    "      // Anchor at the head and rotate DOWN, so the size argument is the bolt",
    "      // LENGTH. Thickness is a fraction of SCREEN height, NOT of the bolt",
    "      // length: deriving it from length made a full-screen bolt a third of a",
    "      // screen THICK, and they merged into one wall of light.",
    "      r.sprite.anchor.set(0, 0.5);",
    "      r.sprite.height = thick;",
    "      r.sprite.position.set(top.x, top.y);",
    "      r.sprite.rotation = Math.PI / 2;",
    "      // GROW the bolt downward instead of placing it at full length.",
    "      //",
    "      // This is what made the original procedural version read as a storm,",
    "      // and dropping it is why the webm version looked simultaneous however",
    "      // the spawn times were arranged: measured mid-run, the procedural",
    "      // bolts were at heights 714/619/406/216/89/18 — a visible cascade of",
    "      // leading edges. A clip placed at full length has no leading edge, so",
    "      // 26 of them 65ms apart are indistinguishable from one flash.",
    "      // Matches the original curve: inQuad, reaching full extent at 0.625 of",
    "      // rainFallMs (the old tween used Math.min(1, v * 1.6)).",
    "      r.sprite.width = fall * 0.06;",
    "      oni.tween({ from: 0, to: 1, duration: cfg.rainFallMs, ease: E.inQuad,",
    "        onUpdate: (v) => { if (!r.sprite.destroyed) r.sprite.width = fall * Math.max(0.06, Math.min(1, v * 1.6)); } });",
    "      if (r.video) { try { r.video.playbackRate = 1 + (Math.random() * 2 - 1) * cfg.rainSpeedJitter; } catch (e) {} }",
    "      // Retire each bolt so the sky does not fill up with every strike that",
    "      // has ever fallen.",
    "      await Promise.race([r.ended, wait(cfg.rainBoltLifeMs)]);",
    "      try { r.sprite.destroy(); } catch (e) {}",
    "      return null;",
    "    }));",
    "    continue;",
    "  }",
