...
function quadElementalSlash(opts = {}) {
  const cfg = Object.assign({
    camZoom: 1.75, camInMs: 1300, camOutMs: 1100,
...
function railStream(opts = {}) {
  const cfg = Object.assign({
    camZoom: 1.55, camInMs: 1400, camOutMs: 1100,
  ...
  const body = [
    ...
    "  host.addChild(g);",
    "  rain.push(wait(i * perBolt).then(() => oni.tween({",
...
function thrash(opts = {}) {
  ...
  const body = [
    ...
    "  const t = ctr(prime);",
    "  const land = { x: home.x + (t.x - home.x) * cfg.standoff, y: home.y + (t.y - home.y) * cfg.standoff };",
    "  await oni.tween({ from: 0, to: 1, duration: cfg.lungeMs, ease: E.inQuad, onUpdate: (v) => {",
...
function stinkyBreath(opts = {}) {
  const cfg = Object.assign({
    ...
    camZoomIn: 1.6, camInMs: 2200,
    camZoomParty: 1.35, camWhipMs: 520,
...
function stripEquip(opts = {}) {
  ...
  const body = [
    ...
    "} });",
    "",
    "// The snatch: a hard metallic glint across the victim.",
    ...
    "const glow = oni.radialTexture('rgba(255,255,255,1)', 'rgba(255,255,255,0)', { size: 96 });",
    "const loot = new PIXI.Sprite(glow);",
    "loot.anchor.set(0.5);",
