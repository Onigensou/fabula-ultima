...
function railStream(opts = {}) {
  const cfg = Object.assign({
    ...
    // Rain
    rainCount: 26, rainWindowMs: 1700, rainFallMs: 480,
    rainHoldMs: 240, rainFadeMs: 260,
    // Bolt shape. jag is the horizontal wander per segment; segments is how many
    // kinks. flickerMs is how often the whole path is re-rolled — this is what
    // makes it crackle rather than sit there.
    rainSegments: 10, rainJag: 24, rainFlickerMs: 45,
    rainCoreWidth: 3, rainBodyWidth: 9, rainAuraWidth: 26,
    rainPulseHz: 9, rainForkChance: 0.35, rainForkLen: 0.28,
    // Earthing burst
    groundCount: 16, groundRadius: 95, groundSize: 11,

    // Per-target strikes (same renderer, heavier)
    hitLeadMs: 420, hitGapMs: 190,
    hitSegments: 12, hitJag: 30, hitCoreWidth: 5, hitBodyWidth: 14, hitAuraWidth: 40,
  ...
  const body = [
    ...
    "  host.addChild(g);",
    "  let pts = jaggedPath(x, yTop, yTop, w.segs, S.wLen(w.jag));",
    "  let lastRoll = 0;",
    "  let earthed = false;",
    ...
    "    if (now - lastRoll >= cfg.rainFlickerMs) { lastRoll = now; pts = jaggedPath(x, yTop, y, w.segs, S.wLen(w.jag)); }",
    "    else { const last = pts[pts.length - 1]; if (last) last.y = y; }",
    "    const pulse = 0.7 + 0.3 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz);",
    "    drawBolt(g, pts, w, pulse, 1);",
    "    if (!earthed && reach >= 1) {",
    "      earthed = true;",
    ...
    "    const now = performance.now();",
    "    if (now - lastRoll >= cfg.rainFlickerMs) { lastRoll = now; pts = jaggedPath(x, yTop, yBot, w.segs, S.wLen(w.jag)); }",
    "    drawBolt(g, pts, w, 0.7 + 0.3 * Math.sin(v * Math.PI * 2 * cfg.rainPulseHz), 1);",
    "  } });",
    "  await oni.tween({ from: 1, to: 0, duration: cfg.rainFadeMs, ease: E.inQuad, onUpdate: (a) => {",
    "    drawBolt(g, pts, w, 0.7, a);",
    "  } });",
    ...
    "  rain.push(wait(i * perBolt).then(() => strike(top.x, top.y, bot.y, rainW, {",
    "    fallMs: cfg.rainFallMs * (0.8 + Math.random() * 0.5),",
    "  })));",
    "}",
    "",
