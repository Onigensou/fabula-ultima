...
function railStream(opts = {}) {
  ...
  const body = [
    ...
    "  if (cfg.rainWebm) {",
    "    rain.push(wait(i * perBolt).then(async () => {",
    "      const L = S.hPx(0.5) * cfg.rainWebmScale;",
...
function stinkyBreath(opts = {}) {
  const cfg = Object.assign({
    ...
    // A PUSH-IN on the Carlbero — the point is to emphasise that it is casting,
    // not to survey the field. >1 moves closer.
    camZoomIn: 1.75, camInMs: 2200,
    camZoomParty: 1.45, camWhipMs: 520,
